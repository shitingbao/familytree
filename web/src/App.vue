<template>
  ...
  <div class="panel">
    ...
    >
      <ul>
        <li>
          <node :person="root" @edit="onEditPerson" />
        </li>
      </ul>
    </div>
  </div>
  <search />
  <edit-person
    v-if="canEdit && currentPerson !== null"
    :person="currentPerson"
    :mode="editMode"
    @close="onClose()"
  />
  <button v-if="canEdit" class="save" @click="onSave()">保存</button>
</template>
...
// axios.get<IPerson>("/tree.json").then((response) => {
...
function getTree() {
  ...
  //         const dataUrl = await axios.post(getQiniuActionUrl(), formData);

  const formData = new FormData();
  formData.append("id", "");
  axios
    .post<IPerson[]>("http://localhost:6200/v1/member/list", formData)
    .then((response) => {
      root.value = response.data[0];
      console.log(" response.data response.data", response.data);
    });
}
